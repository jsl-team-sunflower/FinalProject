...
<%@ include file="../header.jsp"%>
	...
	<div class="container">
	  ...
	  <form name="notice" method="post" action="/qa/write" onsubmit="return check()">
	  <input type="hidden" name="${_csrf.parameterName }" value="${_csrf.token }"/>
	  <!-- post 방식으로 데이터 전송시 CSRF 토큰값을 반드시 같이 보내야 한다 -->
	...
	<script>
		function check() {
			if(notice.qnaWriter.value=="") {
				alert("글쓴이 입력");
				notice.qnaWriter.focus();
				return false;
			}
			if(notice.qnaTitle.value=="") {
				alert("제목을 입력");
				notice.qnaTitle.focus();
				return false;
			}
			if(notice.qnaContent.value=="") {
				alert("내용을 입력");
				notice.qnaContent.focus();
				return false;
			}
			return true;
		}
